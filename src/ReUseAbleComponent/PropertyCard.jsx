...
export default function PropertyCard({ title, LocationSvg, BathroomSvg, location, ModalListerner, Overview, WhishListListener, Property_Name, NavigateListener, img }) {
    const states = useSelector(state => state.AboutTurkey.Nav)

    return (
        <Box className={`listing-card col-12 col-md-6 col-lg-3 margin-ver-1 ${states ? 'z-50' : '-z-50'} `}>
            <Box
                style={{ width: 300 }}
            ...
            >
                <Box className="position-relative">
                    <Link to="/viewproperty" className="card-type btn --circle position-abs card-label raleway border-none opacity-73">
