...
export default function About_Turkey__Tag() {
    ...
    const HandleFetchData = () => {
        const q = query(collection(db, "Contents"), where("key", "==", "Aegean region"));
    ...
    const HandleMarmarRegion = () => {
        const q = query(collection(db, "Contents"), where("key", "==", "Marmara region"));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            const cities = [];
            querySnapshot.forEach((doc) => {
                cities.push({ id: doc.id, ...doc.data() });
    ...
    const FransicoCulture = () => {
        const q = query(collection(db, "Contents"), where("key", "==", "About Sen Francisco Culture"));
        const unsubscribe = onSnapshot(q, (querySnapshot) => {
            const cities = [];
            querySnapshot.forEach((doc) => {
                cities.push({ id: doc.id, ...doc.data() });
                ...
                <Box className="buyer-guide"
                    ...
                    <Box className="row">
                        <Box className="col-12 col-md-3 buyer-guide--side-bar-bg">
                            <Box className="container-fluid">
                                <Typography varient="p" className="buyer-guide--title">
                                    About USA</Typography>
                                <Box className="padding-top-2 side-item">
                                    <Box className="buyer-guide--supertitle">
                                        <Typography varient="p" className="buyer-guide--title-tab" title="Mediterranean region">
                                        About Sen Francisco
                                        </Typography>
                                        {
                                            toggleMedditerrian ? <FiChevronDown onClick={() => settoggleMedditerrian(!toggleMedditerrian)} className=" tabs-icon" aria-hidden="true" /> :
                                                <FiChevronUp onClick={() => settoggleMedditerrian(!toggleMedditerrian)} className="tabs-icon" aria-hidden="true" />

                                        }
                                    </Box>
                                    <ul className={`list-unstyled buyer-guide--ul ${toggleMedditerrian ? '' : 'hide'}`}>
                                        <li className="">
                                ...
                                <Box className="padding-top-2 side-item">
                                    <Box className="buyer-guide--supertitle">
                                        <Typography varient="p" className="buyer-guide--title-tab" title="Aegean region">
                                        About California
                                        </Typography>
                                        {
                                            toggleAegean ? <FiChevronDown className="tabs-icon" onClick={() => settoggleAegean(!toggleAegean)} aria-hidden="true" /> :
                                                <FiChevronUp className="tabs-icon" onClick={() => settoggleAegean(!toggleAegean)} aria-hidden="true" />


                                        }
                                    </Box>
                                    <ul className={`list-unstyled buyer-guide--ul ${toggleAegean ? '' : 'hide'}`}>
                        ...
                        <Box className="col-12  col-md-9  buyer-guide--side-bar-bg-details">
                            <Box className="container-fluid">
                                <PageHeader key={PageHeaderData[0].id} Name={PageHeaderData[0].Name} createdAt={PageHeaderData[0].Created} UpdatedAt={PageHeaderData[0].Updated} />
                                <article className="page-content">
                                    {
                                        ReadContent.filter(item => item.link === 'About California').map((item) => {
                                            return (
                                                <RawHTMLRenderer html={item.value} />
                                            )
                                        })
                                    }
                                </article>
