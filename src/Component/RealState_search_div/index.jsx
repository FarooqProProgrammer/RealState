...
const RealState_search_div = () => {
  ...
  const PropertyFilterTerm = useSelector(state => state.PropertyFilterReducer.Enquiry);
  const PropertyFilterType = useSelector(state => state.typeFilter.Enquiry);
  const PropertyFilterBedroom = useSelector(state => state.BedroomFilter.Phone);
  ...
  const PropertiesData = async () => {
    ...
    const unsubscribe = await onSnapshot(q, (querySnapshot) => {
      ...
      // if (PropertyFilterBedroom) {
      //   console.log(PropertyFilterBedroom)
      //   console.log(cities)
      //   const newFilter = cities.filter(item => item.property_BedRoom+"" === PropertyFilterBedroom+"")
      //   console.log(newFilter)
      //   setProperties(newFilter)
      //   return;
      // }
      ...
      <div className="wrap-dis-9 wrap-tab-9 wrap-mob-9 clearfix mt-3">
        <div className="row cardRow">

          {
            Properties.map((item) => {
              return (
                <ListingCards propertyType={item.Property_Type} item={item} id={item.id} Overview={item.Overview} Property_Living_space={item.Propert_Living_space} Property_Name={item.Property_Name} Area={item.property_Location} Des={item.Overview} title={item.Property_Type} img={item.property_urls[0]} location={item.property_Location} BannerText={item.ImagebannerText} Bathroom={item.Propert_Bathroom} Bedroom={item.property_BedRoom} Property_Price={item.Property_Price} />
              )
            })
          }
          <div className="w-full h-[100px] flex justify-center items-center ">
